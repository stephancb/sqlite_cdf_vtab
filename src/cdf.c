...
*************************************************************************
**
** This file is coding a system of virtual tables and extension functions
**    to create, write and readf cdf files (see https://cdf.gsfc.nasa.gov/)
**
** Example :
**
...
static long cdf_find_epoch(CDFid id) {
    ...
    for( kz=0; kz<nzvars; kz++ ) {
        ...
        if( status!=CDF_OK ) {
            ...
            printf("Cannot get zvar %d data type:\n%s\n", kz, statustext);
            return SQLITE_ERROR;
        }
        ...
        char **pzErr)
{
    ...
    *ppVtab = (sqlite3_vtab*) filevtabp;
    ...
    filevtabp->nsubtabs = 0;

    filevtabp->submodes = sqlite3_malloc(CDF_MAX_NUM_SUBTABS*sizeof(char));
    if( filevtabp->submodes==0 ) {
        rc = SQLITE_NOMEM;
        goto exitlabel;
    }

    filevtabp->names = sqlite3_malloc(CDF_MAX_NUM_SUBTABS*sizeof(char*));
...
static int cdf_close(CdfVTab *pv){
    CDFstatus status = CDFcloseCDF(pv->id);
    if( status!=CDF_OK) {
        char statustext[CDF_STATUSTEXT_LEN+1];
        CDFgetStatusText(status, statustext);
...
static int cdfFileDisconnect(sqlite3_vtab *pvtab){
    ...
    for( int kt=fvp->nsubtabs-1; kt>=0; kt-- ) {
        sqlite3_str_reset(zsql);
        sqlite3_str_appendf(zsql, "DROP TABLE IF EXISTS %s", fvp->names[kt]);
...
/*
** Default destructor for CDF virtual tables.
*/
static int cdfVTabDisconnect(sqlite3_vtab *pvtab){
    CdfVTab *pv = (CdfVTab*) pvtab;
    /* Close the CDF file, unless a subtable: */ 
    int rc = SQLITE_OK;
    if( strchr("st", pv->mode)==NULL )
        rc = cdf_close(pv);
        ...
        char **pzErr)
{
    ...
    sqlite3_str_appendf(zsql, "    sparse INTEGER DEFAULT 0\n");
...
static CDFstatus result_sparse(sqlite3_context *ctx, CDFid id, long kzvar) {
    CDFstatus status;
    long sparse;

    if( (status=CDFgetzVarSparseRecords(id, kzvar-1, &sparse))==CDF_OK )
        sqlite3_result_int64(ctx, sparse);
    else
        sqlite3_result_null(ctx);
    ...
    if( sqlite3_value_type(argv[12])!=SQLITE_NULL ) { /* pad value*/
        ...
        switch (datatype) {
            ...
            case CDF_FLOAT:
                ...
                break;
            case CDF_EPOCH16:
                if( sqlite3_value_bytes(argv[12])!=6 ) {
...
static int zvars_upd_sparse(sqlite3_value **argv, CDFid id, long varnum, char **pzErr) {
    CDFstatus status = CDF_OK;

    if( sqlite3_value_type(argv[13])!=SQLITE_NULL ) { /* sparse */
        status = CDFsetzVarSparseRecords(id, varnum, sqlite3_value_int64(argv[13]));
        if( status<CDF_OK ) {
            char statustext[CDF_STATUSTEXT_LEN+1];
            CDFgetStatusText(status, statustext);
            *pzErr = sqlite3_mprintf("Setting sparse mode for %d failed: %s", varnum, statustext);
            return SQLITE_ERROR;
        }
    }
    return SQLITE_OK;
    ...
    switch (argc) {
        case 1:  /* delete a zVar */
            ...
            if( (rc = cdf_recreate_zrecs(vp))!=SQLITE_OK ) {
                vtabp->zErrMsg = sqlite3_mprintf("zVarsUpdate: recreate_zrecs returned %d\n", rc); 
                return rc;
            }
            break;
        default:  /* create or update a zVar */
            switch( sqlite3_value_type(argv[0]) ) {
                case SQLITE_NULL: /* SQLite INSERT creates a zVar: */
                    ...
                    /* sparse */
                    if( zvars_upd_sparse(argv, vp->id, varnum, &vtabp->zErrMsg)!=SQLITE_OK )
                        return SQLITE_ERROR;
                ...
                default:  /* SQLite UPDATE can rename a zVariable, change the max allocated record or the pad value: */
                    ...
                    if( change ) {
                        vtabp->zErrMsg = sqlite3_mprintf(
                                "zVar can be be renamed, or only max allocated records or pad value or sparse updated");
                        return SQLITE_ERROR;
                    }
                    ...
                    /* sparse */
                    if( !sqlite3_value_nochange(argv[13])
                            && zvars_upd_sparse(argv, vp->id, varnum, &vtabp->zErrMsg)!=SQLITE_OK )
                            return SQLITE_ERROR;
            }
    }
    return SQLITE_OK;
...
static void cdf_seqpos(CDFid id, long kz, long kr) {
    long recnum,indices[2];
    CDFstatus status = CDFgetzVarSeqPos(id, kz, &recnum, indices);

    if( recnum!=kr )
        status = CDFsetzVarSeqPos(id, kz, kr, indices);
}
...
static int cdfzRecsUpdate(sqlite3_vtab *vtabp, int argc, sqlite3_value **argv, sqlite_int64 *rowid ) {
    ...
    switch (argc) {
        ...
        default:  /* insert or replace or update */
            if( argc-2!=nzvars+1 ) {
                *pzErr = sqlite3_mprintf("Nr of columns %d is not equal nr of zVars %d!",
                                         argc-2, nzvars+1);
                return SQLITE_ERROR;
            }
        ...
        char **pzErr)
{
    ...
    if( mode!='r' && mode!='s' ) {
        *pzErr = sqlite3_mprintf("The CDFRead table must be readonly");
        return SQLITE_READONLY;
    }
        ...
        char **pzErr)
{
    ...
    rc = cdf_declare_vtab(db, zsql, pzErr);
    if( rc !=SQLITE_OK ) {
        *pzErr = sqlite3_mprintf("Error %d declaring Epochs vtab", rc);
        return rc;
    }
...
static void cdfEpoch( sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if( argc==2 || argc==3 ) {
        sqlite3_result_double(ctx, ILLEGAL_EPOCH_VALUE);
        return;
    }
    if( argc==1 )
        epoch = toParseEPOCH((char*) sqlite3_value_text(argv[0]));
    else if( argc>=4 ) {
        hour = sqlite3_value_int64(argv[3]);
        if( argc>=5 ) {
            minute = sqlite3_value_int64(argv[4]);
            if( argc>=6 ) {
                second = sqlite3_value_int64(argv[5]);
                if( argc>=7 )
                    millisec = sqlite3_value_int64(argv[6]);
            }}
    
        epoch = computeEPOCH(sqlite3_value_int64(argv[0]), sqlite3_value_int64(argv[1]), sqlite3_value_int64(argv[2]),
                hour, minute, second, millisec);
    }
    sqlite3_result_double(ctx, epoch);
}

static void cdfEpoch1( sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    double epoch = parseEPOCH1((char*) sqlite3_value_text(argv[0]));
    sqlite3_result_double(ctx, epoch);
}

static void cdfEpoch2( sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    double epoch = parseEPOCH2((char*) sqlite3_value_text(argv[0]));
    sqlite3_result_double(ctx, epoch);
}

static void cdfencodeTT2000( sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    char *iso8601 = sqlite3_malloc(32);
    memset(iso8601, 0, 32);
    encodeTT2000(sqlite3_value_int64(argv[0]), iso8601);
    sqlite3_result_text(ctx, iso8601, -1, sqlite3_free);
}

static void cdfencodeTT2000s( sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    char *iso8601 = sqlite3_malloc(32);
    memset(iso8601, 0, 32);
    encodeTT2000(sqlite3_value_int64(argv[0]), iso8601, sqlite3_value_int(argv[1]));
    sqlite3_result_text(ctx, iso8601, -1, sqlite3_free);
}
...
int sqlite3_cdf_init(
  ...
  rc = sqlite3_create_function(
          db, "cdfEpoch", -1, SQLITE_UTF8 | SQLITE_DETERMINISTIC | SQLITE_INNOCUOUS,
          0 /* no user data */, cdfEpoch, 0, 0);
  if( rc!=SQLITE_OK ) return rc;

  rc = sqlite3_create_function(
          db, "cdfEpoch1", 1, SQLITE_UTF8 | SQLITE_DETERMINISTIC | SQLITE_INNOCUOUS,
          0 /* no user data */, cdfEpoch1, 0, 0);
  if( rc!=SQLITE_OK ) return rc;

  rc = sqlite3_create_function(
          db, "cdfEpoch2", 1, SQLITE_UTF8 | SQLITE_DETERMINISTIC | SQLITE_INNOCUOUS,
          0 /* no user data */, cdfEpoch2, 0, 0);
  if( rc!=SQLITE_OK ) return rc;

  rc = sqlite3_create_function(
          db, "encodeTT2000", 1, SQLITE_UTF8 | SQLITE_DETERMINISTIC | SQLITE_INNOCUOUS,
          0 /* no user data */, cdfencodeTT2000, 0, 0);
  if( rc!=SQLITE_OK ) return rc;

  rc = sqlite3_create_function(
          db, "encodeTT2000", 2, SQLITE_UTF8 | SQLITE_DETERMINISTIC | SQLITE_INNOCUOUS,
          0 /* no user data */, cdfencodeTT2000s, 0, 0);
